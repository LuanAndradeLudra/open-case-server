module.exports = (function () {
  ...
  LootTable.prototype.choose = function () {
    ...
    let choice = 0;
    const randomNumber = generatedMethod2(totalWeight);
  ...
  function generatedMethod1(totalWeight) {
    return Math.floor(Math.random() * totalWeight);
  }
  function generatedMethod2(totalWeight) {
    const precision = 1000;
    return (
      Math.floor(Math.random() * (totalWeight * precision)) / (1 * precision)
    );
  }
